...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _saveSymptomLog({
    ...
    // Send to backend
    _postToBackend('/cycle/log', {
      'user_id': 'user_001',
      'period_start': date.toIso8601String().split('T')[0],
      'period_end': date.toIso8601String().split('T')[0],
      'symptoms': symptoms,
      'mood': mood,
      'stress_level': 2,
      'sleep_hours': 7.0,
  ...
  void _applyPrediction(
    ...
    try {
      ...
      setState(() {
        _nextPeriod = data['next_period_start']?.toString() ?? _fmt(nextStart!);
        _fertileWindow = data['fertile_window']?.toString() ?? '—';
        _ovulationDay = data['ovulation_day']?.toString() ?? '—';
        _cycleLength =
            '${(data['predicted_cycle_length'] ?? avgCycle).toStringAsFixed(0)} days';
        _currentPhase = data['current_phase']?.toString() ?? '—';
        _phaseDesc =
  ...
  void _useFallback(double avgCycle, double avgDuration) {
    ...
    setState(() {
      _nextPeriod = _fmt(nextStart);
      _fertileWindow = '${_fmtShort(fertileStart)} – ${_fmt(fertileEnd)}';
      _ovulationDay = _fmt(ovulation);
      _cycleLength = '${avgCycle.round()} days';
      _currentPhase = phase;
      _phaseDesc = desc;
      _daysUntilPeriod = days;
